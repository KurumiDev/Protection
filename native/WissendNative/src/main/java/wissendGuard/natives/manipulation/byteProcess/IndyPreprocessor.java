...
public class IndyPreprocessor implements Preprocessor {
    ...
    private static void processIndy(ClassNode classNode, MethodNode methodNode,
                                    InvokeDynamicInsnNode invokeDynamicInsnNode, StartPlatform startPlatform) {
            ...
            case STD_JAVA: {
                ...
                if (bsmArguments.length < 3 || !bsmArguments[0].getDescriptor().equals("Ljava/lang/invoke/MethodHandles$Lookup;") ||
                    ...
                    resultInstructions.add(new MethodInsnNode(Opcodes.INVOKESPECIAL, "java/lang/BootstrapMethodError",
                            "<init>", "OBF((Ljava/lang/String;)V)")); // 1
                ...
                bootstrapInstructions.add(new TypeInsnNode(Opcodes.ANEWARRAY, "OBF(java/lang/Object)")); // 1
                {
    ...
    @Override
    public void process(ClassNode classNode, MethodNode methodNode, StartPlatform startPlatform) {
        for (int i = 0; i < methodNode.instructions.size(); i++) {
            AbstractInsnNode insnNode = methodNode.instructions.get(i);
            if (insnNode instanceof InvokeDynamicInsnNode) {
                processIndy(classNode, methodNode, (InvokeDynamicInsnNode) insnNode, startPlatform);
            }
        }
    }
}
