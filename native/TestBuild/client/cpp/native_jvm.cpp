...
namespace native_jvm::utils {
    ...
    void init_utils(JNIEnv *env) {
        jclass clazz = env->FindClass(VMProtectDecryptStringA("[Z"));
        if (env->ExceptionCheck())
            return;
        boolean_array_class = (jclass) env->NewGlobalRef(clazz);
        env->DeleteLocalRef(clazz);

        jclass string_clazz = env->FindClass(VMProtectDecryptStringA("java/lang/String"));
        if (env->ExceptionCheck())
            return;
        string_intern_method = env->GetMethodID(string_clazz, VMProtectDecryptStringA("intern"), VMProtectDecryptStringA("()Ljava/lang/String;"));
        if (env->ExceptionCheck())
            return;
        env->DeleteLocalRef(string_clazz);

        jclass _class_class = env->FindClass(VMProtectDecryptStringA("java/lang/Class"));
        if (env->ExceptionCheck())
            return;
        class_class = (jclass) env->NewGlobalRef(_class_class);
        env->DeleteLocalRef(_class_class);

        get_classloader_method = env->GetMethodID(class_class, VMProtectDecryptStringA("getClassLoader"), VMProtectDecryptStringA("()Ljava/lang/ClassLoader;"));
        if (env->ExceptionCheck())
            return;

        jclass _object_class = env->FindClass(VMProtectDecryptStringA("java/lang/Object"));
        if (env->ExceptionCheck())
            return;
        object_class = (jclass) env->NewGlobalRef(_object_class);
        env->DeleteLocalRef(_object_class);

        get_class_method = env->GetMethodID(object_class, VMProtectDecryptStringA("getClass"), VMProtectDecryptStringA("()Ljava/lang/Class;"));
        if (env->ExceptionCheck())
            return;

        jclass _classloader_class = env->FindClass(VMProtectDecryptStringA("java/lang/ClassLoader")); //
        if (env->ExceptionCheck())
            return;
        classloader_class = (jclass) env->NewGlobalRef(_classloader_class);
        env->DeleteLocalRef(_classloader_class);

        load_class_method = env->GetMethodID(classloader_class, VMProtectDecryptStringA("loadClass"), VMProtectDecryptStringA("(Ljava/lang/String;)Ljava/lang/Class;"));
        if (env->ExceptionCheck())
            return;

        jclass _no_class_def_found_class = env->FindClass(VMProtectDecryptStringA("java/lang/NoClassDefFoundError"));
        if (env->ExceptionCheck())
            return;
        no_class_def_found_class = (jclass) env->NewGlobalRef(_no_class_def_found_class);
        env->DeleteLocalRef(_no_class_def_found_class);

        ncdf_init_method = env->GetMethodID(no_class_def_found_class, VMProtectDecryptStringA("<init>"), VMProtectDecryptStringA("(Ljava/lang/String;)V"));
        if (env->ExceptionCheck())
            return;

        jclass _throwable_class = env->FindClass(VMProtectDecryptStringA("java/lang/Throwable"));
        if (env->ExceptionCheck())
            return;
        throwable_class = (jclass) env->NewGlobalRef(_throwable_class);
        env->DeleteLocalRef(_throwable_class);

        get_message_method = env->GetMethodID(throwable_class, VMProtectDecryptStringA("getMessage"), VMProtectDecryptStringA("()Ljava/lang/String;"));
        if (env->ExceptionCheck())
            return;

        init_cause_method = env->GetMethodID(throwable_class, VMProtectDecryptStringA("initCause"),
                                            VMProtectDecryptStringA("(Ljava/lang/Throwable;)Ljava/lang/Throwable;"));
        if (env->ExceptionCheck())
            return;

        jclass _methodhandles_lookup_class = env->FindClass(VMProtectDecryptStringA("java/lang/invoke/MethodHandles$Lookup"));
        if (env->ExceptionCheck())
            return;
        ...
        link_call_site_method = env->GetStaticMethodID(methodhandle_natives_class, VMProtectDecryptStringA("linkCallSite"),
            VMProtectDecryptStringA("(Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/invoke/MemberName;"));
        is_jvm11_link_call_site = false;
        if (env->ExceptionCheck()) {
            env->ExceptionClear();
            link_call_site_method = env->GetStaticMethodID(methodhandle_natives_class, VMProtectDecryptStringA("linkCallSite"),
                VMProtectDecryptStringA("(Ljava/lang/Object;ILjava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/invoke/MemberName;"));
    ...
    template <>
    jarray create_array_value<OBF_ADD(-4, 8)>(JNIEnv *env, jint size) {
        return env->NewShortArray(size);
    }

    template <>
    jarray create_array_value<OBF_ADD(2, 3)>(JNIEnv *env, jint size) {
        return env->NewIntArray(size);
    }

    template <>
    jarray create_array_value<OBF_ADD(0, 6)>(JNIEnv *env, jint size) {
        return env->NewFloatArray(size);
    }

    template <>
    jarray create_array_value<OBF_ADD(1, 6)>(JNIEnv *env, jint size) {
        return env->NewLongArray(size);
    }

    template <>
    jarray create_array_value<OBF_ADD(6, 2)>(JNIEnv *env, jint size) {
    ...
    jobjectArray create_multidim_array(JNIEnv *env, jobject classloader, jint count, jint required_count,
        ...
        if (required_count == OBF_ADD(-128, 128)) {
            env->FatalError(VMProtectDecryptStringA("required_count == 0"));
            return nullptr;
        }
        jint current_size = sizes.begin()[dim_index];
        if (current_size < OBF_ADD(-90, 90)) {
            throw_re(env, VMProtectDecryptStringA("java/lang/NegativeArraySizeException"), VMProtectDecryptStringA("MULTIANEWARRAY size < 0"), line);
            return nullptr;
        }
        jobjectArray result_array = nullptr;
        if (count == OBF_ADD(-4, 5)) {
        ...
        std::string clazz_name = std::string(count - OBF_ADD(0, 1), '[') + "L" + std::string(class_name) + ";";
        ...
        for (jint i = 0; i < current_size; i++) {
            jobjectArray inner_array = create_multidim_array(env, classloader, count - OBF_ADD(-2, 3), required_count - OBF_ADD(-3, 4),
                class_name, line, sizes, dim_index + OBF_ADD(-1, 2));
            if (env->ExceptionCheck()) {
