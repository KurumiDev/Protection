...
public final class AntiTamper {
    ...
    public static void enforce() {
        ...
        try {
            if (detectDebugger()) {
                System.err.println("[AntiTamper] Debugger detected");
                fail();
            }
            if (detectTimingBreakpoint()) {
                System.err.println("[AntiTamper] Timing breakpoint detected");
                fail();
            }
            if (detectVirtualMachine()) {
                System.err.println("[AntiTamper] VM/Sandbox detected");
                fail();
            }
        } catch (Throwable t) {
            System.err.println("[AntiTamper] Probe check failed: " + t);
            fail();
        }
    }
    ...
    public static void quickCheck() {
        if (disabled()) return;
        if (detectDebugger()) {
            System.err.println("[AntiTamper] QuickCheck: debugger detected");
            fail();
        }
    }
