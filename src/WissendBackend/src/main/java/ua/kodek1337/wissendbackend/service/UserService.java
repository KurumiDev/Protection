...
import ua.kodek1337.wissendbackend.repo.KeysRepo;
...
public class UserService {
    ...
    public ResponseEntity<?> repass(String name, String oldPass, String newPass) {
        UserModule userModule = userRepo.findByUsername(name).get();

        if (userModule.getPassword().equals(bCryptPasswordEncoder.encode(oldPass))) {
            userModule.setPassword(bCryptPasswordEncoder.encode(newPass));
            userRepo.save(userModule);
            return ResponseEntity.status(200).body(Map.of("Success",  "Password updated!"));
        } else {
            return ResponseEntity.status(403).body(Map.of("Error", "password not exits!"));
        }
    }
}